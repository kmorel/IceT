...
int* radixkGetK(int world_size, int* r) {
  /* Divide the world size into groups that are closest to the magic
     k value. */
  int* k = NULL;
  int num_groups = 0;
...
void radixkGatherFinalImage(IceTInt* compose_group, IceTInt group_rank,
  ...
  if (color_format != ICET_IMAGE_COLOR_NONE) {
    ...
    if (group_rank == image_dest) {
      for (i = 0; i < group_size; i++) {
        if (i != group_rank) {
          requests[i] = icetCommIrecv(color_buf + pixel_size * all_offsets[i],
                                      pixel_size * all_sizes[i], ICET_BYTE,
                                      compose_group[i], SWAP_IMAGE_DATA);
        } else {
          requests[i] = ICET_COMM_REQUEST_NULL;
        }
  ...
  if (depth_format != ICET_IMAGE_DEPTH_NONE) {
    ...
    if (group_rank == image_dest) {
      for (i = 0; i < group_size; i++) {
        if (i != group_rank) {
          requests[i] = icetCommIrecv(depth_buf + pixel_size * all_offsets[i],
                                      pixel_size * all_sizes[i], ICET_BYTE,
                                      compose_group[i], SWAP_IMAGE_DATA);
        } else {
          requests[i] = ICET_COMM_REQUEST_NULL;
...
void icetRadixkCompose(IceTInt *compose_group, IceTInt group_size,
  ...
  int* k = radixkGetK(group_size, &r); /* Communication sizes for each round. */
  ...
  /* TODO: If high k-values only appear later in the algorithm then we
     don't need max_k bufs of size/k[0] length. We can allocate smaller
     buffers for the later rounds. */
  for (i = 0; i < max_k - 1; i++) {
  ...
  /* Any peer we communicate with in round i starts that round with a block of
     ...
     calculate the current round's peer sizes based on our current size and the
     k[i] info. */
  my_size = size;
...
#define MAP_BUF(k_rank_) ((k_rank_) - ((k_rank_) > rv[i] ? 1 : 0))
  ...
  for (i = 0; i < r; i++){
    ...
    for (j = 1; j < k[i]; j++) {
      ...
      if (dest < k[i]) {
        r_reqs[n] = icetCommIrecv(recv_bufs[MAP_BUF(dest)], max_sparse_img_size,
                                  ICET_BYTE, compose_group[partners[dest]], 0);
        dests[n++] = dest;
      }
    ...
    for (j = 1; j < k[i]; j++) {
      ...
      if (dest < k[i]) {
        ...
        s_reqs[n++] = icetCommIsend(package_buffer, package_size, ICET_BYTE,
                                    compose_group[partners[dest]], 0);
      }
      dest = rv[i] - j; /* Toward lower group members */
      if (dest >= 0) {
        icetCompressSubImage(image, ofsts[dest], sizes[dest],
    ...
    for(n = 0; n < k[i]; n++) {
      ...
      for (j = 1; j < k[i]; j++) {
        /* --------  up direction ----------- */
        if ((dest = rv[i] + j) < k[i]) {
          /* distance from dest to start of already composited region */
          b = 1;
          ...
          if (here[dest] && !done[dest] && here[dest - b] 
            ...
            IceTSparseImage inSparseImage
              = icetSparseImageUnpackageFromReceive(recv_bufs[MAP_BUF(dest)]);
            icetCompressedSubComposite(image, my_ofst, inSparseImage, 0);
            done[dest] = 1;
          }
          ...
          if (dest - b > rv[i] && here[dest] && here[dest - b] &&
            ...
            /* Unpackage and uncompress one of the images. This is for making
               use of IceT's compressed compositing operations. Once a
               function for compositing two compressed images is available,
               this can be changed. */
            IceTSparseImage firstSparseImage =
              icetSparseImageUnpackageFromReceive(recv_bufs[MAP_BUF(dest - b)]);
            icetDecompressImage(firstSparseImage, intermediate_compose_img);
            IceTSparseImage secondSparseImage =
              icetSparseImageUnpackageFromReceive(recv_bufs[MAP_BUF(dest)]);

            /* Composite. Start at offset 0 since the image we uncompressed
               is a subimage. */
            icetCompressedSubComposite(intermediate_compose_img, 0,
                                       secondSparseImage, 0);

            /* Compress the composited image back into the original buffer. */
            icetCompressSubImage(intermediate_compose_img, 0, my_size,
                                 firstSparseImage);
            icetSparseImagePackageForSend(firstSparseImage,
        ...
        if ((dest = rv[i] - j) >= 0) {
          ...
          if (here[dest] && !done[dest] && here[dest + b] 
            ...
            IceTSparseImage inSparseImage
              = icetSparseImageUnpackageFromReceive(recv_bufs[MAP_BUF(dest)]);
            icetCompressedSubComposite(image, my_ofst, inSparseImage, 1);
            done[dest] = 1;
          }
          ...
          if (dest + b < rv[i] && here[dest] && here[dest + b] &&
            ...
            IceTSparseImage secondSparseImage =
              icetSparseImageUnpackageFromReceive(recv_bufs[MAP_BUF(dest)]);
            /* Start compositing at offset 0 because we just decompressed a
               subimage. */
            icetCompressedSubComposite(intermediate_compose_img, 0,
                                       secondSparseImage, 1);

            /* Compress the composited image back into the original buffer.
               Similarly, the offset is zero is it is intially a subimage. */
            icetCompressSubImage(intermediate_compose_img, 0, my_size,
                                 firstSparseImage);
            icetSparseImagePackageForSend(firstSparseImage,
                                          &(recv_bufs[MAP_BUF(dest + b)]), 
